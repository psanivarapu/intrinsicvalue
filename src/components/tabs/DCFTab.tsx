...
export function DCFTab() {
  ...
  const inputs = canCompute ? {
    baseCashFlow: baseCF, stage1Growth: stage1, stage2Growth: stage2,
    terminalGrowth: termG, discountRate: r, netDebtAdjustment: netDebtAdj, sharesOutstanding: shares,
  } : null
      ...
      <div className="grid grid-cols-3 gap-4 items-start">
        ...
        <div className="col-span-2 border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          {!isAdv && (
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                <NumberInput
                  label="Base Free Cash Flow" unit="₹ Cr"
                  ...
                  tooltip={<div>OCF minus Capex. Screener → <strong>Cash Flow</strong> tab.</div>}
                  compact
                />
                <NumberInput
                  label="Growth (yrs 1–10)" unit="%"
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500">
                  ...
                  {isFCFF && !derived.wacc && <span className="ml-1 text-amber-600">⚠ set beta + market cap for WACC</span>}
                </span>
              </div>
              ...
              {isFCFF && (
                <>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {([
                      ['ebit', 'EBIT', pl],
                      ['depreciationAmortization', 'D&A', cf],
                      ['capex', 'Capex', cf],
                      ['changeInWorkingCapital', 'ΔWC', cf],
                  ...
                  <div className="grid grid-cols-2 gap-2 items-center">
                    ...
                    />
                    {baseCF !== 0 && (
                      <div className="text-xs px-2 py-1.5 bg-indigo-50 border border-indigo-100 rounded text-indigo-700">
                        FCFF: <strong>{fmtCr(baseFCFF)}</strong>
                      </div>
                    )}
                  </div>
                </>
              ...
              {!isFCFF && (
                <>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {([
                      ['operatingCashFlow', 'Op. Cash Flow', cf],
                      ['capex', 'Capex', cf],
                  ...
                  {baseCF !== 0 && (
                    <div className="text-xs px-2 py-1 bg-indigo-50 border border-indigo-100 rounded text-indigo-700 inline-block">
                      FCFE: <strong>{fmtCr(baseFCFE)}</strong>
                    </div>
                  )}
                </>
              )}
              ...
              <div className="grid grid-cols-2 gap-2 items-start">
                ...
                <NumberInput label="Sector EV/EBITDA norm" value={dcf.sectorNormEVEBITDA}
                  ...
                  placeholder="e.g. 20"
                  tooltip={<div>Used to cross-check terminal value via implied exit multiple.</div>}
                  compact
                />
              </div>
          ...
          {validation.errors.map((e, i) => (
            <div key={i} className="p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700">{e.message}</div>
          ))}
          {validation.warnings.map((w, i) => (
            <div key={i} className="p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-700">{w.message}</div>
          ))}
        </div>
                ...
                <table className="text-xs w-full border-collapse">
                  <thead>
                    <tr>
                      ...
                      {[-2, 0, 2].map(g => (
                        <th key={g} className="text-center text-gray-600 p-0.5 font-medium">g{g >= 0 ? '+' : ''}{g}%</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
